 ...
 * Departamento de Inform谩tica de Sistemas y Computadores (DISCA)
 * Universidad Polit猫cnica de Valencia.
 *
 * Autor: Pedro L贸pez (plopez@disca.upv.es)
 *        Sergio S谩ez (ssaez@disca.upv.es)
 *
 * Fichero: mips.c
 *
 * Descripci贸n:
...
void fase_busqueda(
    ...
    init_instruc(PC, orden); /*** VISUALIZACI脫N ***/

    /* Busca la siguiente instrucci贸n */

    if (en_region(mmem, PC, r_text)) {
        if (esta_alineada_dir_instruc(PC)) {
            IF_IDn.IR = lee_mem_instruc(mmem, PC); /* Busca la siguiente instrucci贸n */
        } else {
            /* Direcci贸n de acceso a memoria de instrucciones desalineada */
            fprintf(stderr, "\nAtenci髇 (IF): acceso desalineado al segmento de instrucciones Ciclo=%u PC=%"PRId64"\n", Ciclo + 1, PC);
        }
    } else {
        /*** Excepci贸n en el acceso a la memoria de instrucciones */
        fprintf(stderr, "\nAtenci髇 (IF): acceso fuera del segmento de instrucciones Ciclo=%u PC=%"PRId64"\n", Ciclo + 1, PC);

    } /* endif */

    IF_IDn.NPC = PC + 4;

    IF_IDn.iPC = PC; /* Visualizaci贸n */
    IF_IDn.orden = orden;
...
int fase_decodificacion() {
    ...
    if inst_int(IF_ID.IR.codop) {
        ...
        // Esta parte solo ser铆a necesaria si el banco de registros entero lo pudieran escribir
        // dos instrucciones enteras simult谩neamente.
        //
        // Se producir谩 una escritura en 3 ciclos
        ...
        /* A diferencia de la unidad de 5 etapas, no podemos dejar que la instrucci贸n
         * contn煤e, ya que contaminar铆a el campo busy del Rfp */
        if (IDstall == NO) {
            ...
            switch (IF_ID.IR.codop) {
                ...
                default:
                    fnError("Instrucci贸n %d no implementada\n", IF_ID.IR.codop)
                            // fprintf(stderr,
                            //         "ERROR (%d %d): Instrucci贸n no implementada\n",
...
int fase_escritura(
    ...
    if (final) {
        fprintf(stderr, "\nCiclo %u: El programa est谩 terminando normalmente\n", Ciclo + 1);
    }
    return (final);
...
void fase_escritura_FP(
    ...
    switch (FP_WB.IR.codop) {
        ...
        case OP_FP_S_PS:
            break;
        default:
            fnError("La instrucci贸n %d no deber铆a estar en FP WB\n", FP_WB.IR.codop)
                    //fprintf(stderr,
                    //        "ERROR (%d %d): Esta instrucci贸n no deberia estar en FP WB\n",
...
void fase_escritura_FPSR(
    ...
    switch (CMP_WB.IR.codop) {
        ...
        case OP_NOP:
            break;
        default:
            fnError("La instrucci贸n %d no deber铆a estar en FPSR WB\n", CMP_WB.IR.codop)
                    //fprintf(stderr,
                    //        "ERROR (%d %d): Esta instrucci贸n no deberia estar en FPSR WB\n",
...
void impulso_reloj() {
    ...
    if (!IFstall) {
        orden = orden + 1; /* VISUALIZACI脫N */
        PC = PCn;
    } else {
        if (SaltoEfectivo) {
            /* est谩 activado IDstall por culpa de IDstall y hay un salto en este ciclo */
