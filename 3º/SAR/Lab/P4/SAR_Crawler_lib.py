...
class SAR_Wiki_Crawler:
    ...
    def parse_wikipedia_textual_content(self, text: str, url: str) -> Optional[Dict[str, Union[str,List]]]:
        ...
        groups = match.groupdict()              # Diccionario con las keys ['title', 'summary', 'rest']
        ...
        # TODO refactorizar esto en método para sección
        for s in section_tuples_list:
            ...
            section_dic = match.groupdict()     # Diccionario con las keys ['title', 'summary', 'rest']
    ...
    def start_crawling(self, 
        ...
        url = hq.heappop(queue)[2] # Esto devuelve una tupla de la que la url es el tercer componente

        # 2. Descarga el contenido textual de la página y los enlaces que aparecen en ella
        content = self.get_wikipedia_entry_content(url)[0]

        # 3. Añadir, si procede, los enlaces a la cola de páginas pendientes de procesar
            # no se ha visitado
            # es url valida
            # hemos llegado a profundidad máxima?
        
        # 4. Analizar el contenido textual para generar el diccionario con el contentenido estructurado del artículo
        # TODO lo de abajo es temporal, seguramente no se guarde así
        document = self.parse_wikipedia_textual_content(content, url)
        self.save_documents([document], base_filename)
