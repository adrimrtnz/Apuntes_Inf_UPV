...
> Buscar la definición de esto
$$
\mathcal{L} \subseteq \Sigma^*, x \in \Sigma^* \\
x^{-1}\mathcal{L} = \{y \in \Sigma^* : xy \in \mathcal{L}\} \\
\mathcal{L} = \{aba, abbb, babb, ababb,\} \\
ab^{-1}\mathcal{L} = \{a, bb, abb \} \rightarrow \text{Aquí sería qué podemos poner a la derecha de ab para obtener expresiones definidas en } \mathcal{L} \\ \\
...
Todas aquellas palabras que concatenadas a la izquierda de $x$ dan como resultado una palabra de $\Sigma$ .
$$
\mathcal{L} x^{-1} = \{y \in \Sigma^* : yx \in \mathcal{L} \}
$$
...
h(\mathcal{L}) = \{aabc, aabb, aacc \} \\ \\

h(a) = 0, h(b) = 01 \\
h(\{a,b\}^*) = \{x \in \{0,1\}^* \} : x \not \in PREF(1) \wedge 11 \not \in SEG(x) \}
$$
