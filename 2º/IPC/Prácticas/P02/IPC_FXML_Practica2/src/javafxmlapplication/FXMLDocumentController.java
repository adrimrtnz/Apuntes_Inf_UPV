...
public class FXMLDocumentController implements Initializable {
    ...
    private void handleKeyPressed(KeyEvent event) {
        ...
        GridPane grid = (GridPane)circleFigure.getParent();
        
        if(event.getCode() == KeyCode.UP) {
            GridPane.setRowIndex(circleFigure, rowNorm(grid, row - 1));
        }
        else if(event.getCode() == KeyCode.DOWN) {
            GridPane.setRowIndex(circleFigure, rowNorm(grid, row + 1));
        }
        else if(event.getCode() == KeyCode.LEFT) {
            GridPane.setColumnIndex(circleFigure, columnNorm(grid, column - 1));
        }
    ...
    private void handleMousePressed(MouseEvent event) {
        ...
        GridPane.setConstraints(circleFigure, columnCalc(grid, x), rowCalc(grid, y));
    }
