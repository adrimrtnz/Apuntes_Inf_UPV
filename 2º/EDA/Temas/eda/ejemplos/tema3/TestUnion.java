...
public class TestUnion {
    ...
    public static <E> String unionMap(ListaConPI<E> l1, ListaConPI<E> l2) {
        ListaConPI<EntradaHash<E,Integer>>[] aux = new LEGListaConPI[l1.talla() + l2.talla()];
        for (l1.inicio(); !l1.esFin(); l1.siguiente()) {
            E e = l1.recuperar();
            for (aux.inicio(); 
                !aux.esFin() && !e.equals(aux.recuperar().valor); 
                aux.siguiente());
            if (aux.esFin()) { aux.insertar(e, 1); } 
            else {  aux.recuperar().valor++; }
        }
        for (l2.inicio(); !l2.esFin(); l2.siguiente()) {
            E e = l2.recuperar();
            for (aux.inicio(); 
